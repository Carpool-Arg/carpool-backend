...
public class TripImplementation implements ITripService {
    ...
    public Response<Void> createTrip(TripRequestDTO tripRequestDTO) {
        ...
        if (authenticatedDriver.getLicenseStatus() != LicenseStatusEnum.APPROVED) {
            throw new ForbiddenException(
                    "No podés publicar viajes hasta que tu carnet de conducir sea verificado y aprobado.");
        }

        // Validar que el carnet no esté vencido
        if (authenticatedDriver.getLicenseExpirationDate().isBefore(LocalDate.now())) {
            throw new ForbiddenException(
                    "Tu carnet de conducir se encuentra vencido. Por favor, actualizá tu información.");
        }

        if (authenticatedDriver.getLicenseExpirationDate().isBefore(tripRequestDTO.getStartDateTime().toLocalDate())) {
            throw new ForbiddenException(
                    "Tu carnet de conducir se va a encontrar vencido para la fecha de inicio del viaje. Por favor, actualizá tu información.");
        }
        ...
        if (tripRepository.hasOverlappingSchedule(authenticatedDriver.getUser().getId(), authenticatedDriver.getId(),
                tripRequestDTO.getStartDateTime(),
                newEnd, null)) {
    ...
    public Response<TripDriverResponseDTO> getTrips(List<String> tripState, int skip) {
        ...
        Page<Trip> tripsPage = tripRepository.findTripsByDriverIdWithCurrentStateTrip(driver.getId(), tripState,
                pageable);
    ...
    @Override
    public Response<Void> checkTripAvailability(LocalDateTime startDateTime, Long idTrip, Long idOriginCity,
            Long idDestinationCity) {
        ...
        Double distance = CoordsUtils.calculateDistance(originCity.getLatitude(), originCity.getLongitude(),
        ...
        if (estimatedEndTime.isBefore(LocalDateTime.now())) {
            log.warn(
                    "La fecha y hora de inicio estimada para este viaje ya ha pasado. startDateTime={}, estimatedEndTime={}",
                    startDateTime, estimatedEndTime);
            throw new ConflictException(
                    "La fecha y hora de inicio estimada para este viaje ya ha pasado. Por favor, elige una fecha y hora futuras.");
        }

        if (tripRepository.hasOverlappingSchedule(driver.getUser().getId(), driver.getId(), startDateTime,
                estimatedEndTime, idTrip)) {
            log.warn(
                    "El viaje no está disponible. Se superpone con otro viaje activo del chofer. startDateTime={}, estimatedEndTime={}, driverId={}, idTrip={}",
                    startDateTime, estimatedEndTime, driver.getId(), idTrip);
    ...
    public Response<List<TripSearchResponseDTO>> getInitialFeed(Long userCityId, int skip) {
        ...
        Page<Trip> tripsPage = tripRepository.findTripsForInitialFeed(userCityId, userId, LocalDateTime.now(),
                pageable);
    ...
    public Response<TripHistoryUserResponseDTO> getHistoryTripUser(List<String> namesStateTrip, int skip) {
        ...
        if (existingStates.size() != namesStateTrip.size()) {
            List<String> invalidStates = namesStateTrip.stream().filter(state -> !existingStates.contains(state))
                    .toList();

            throw new BadRequestException("Estados inválidos: " + String.join(", ", invalidStates));
        }

        Page<Reservation> reservationsPage = reservationRepository.findUserReservationsByTripState(userId,
                namesStateTrip, getPageable(skip));
        ...
        List<TripHistoryUserDTO> tripDtos = reservations.stream()
                .map(reservation -> tripMapper.convertTripToHistoryDTO(reservation))
                .toList();
        log.info("Operacion completada con exito");

        TripHistoryUserResponseDTO response = TripHistoryUserResponseDTO.builder()
                .trips(tripDtos)
                .build();
    ...
    public Response<TripPriceCalculationResponseDTO> calculatePublishSeatPrice(Double seatPrice,
        ...
        ConfigParameters config = parametersRepository
                .findByKeyName(ParametersEnum.DISCOUNT_PERCENTAGE.getKey())
                .orElseThrow(() -> new ResourceNotFoundException(
                        String.format(CONFIG_NOT_FOUND, ParametersEnum.DISCOUNT_PERCENTAGE.getKey())));

        double commissionPercentage = Double.parseDouble(config.getKeyValue());

        TripPriceCalculationResponseDTO calculation = tripMapper.convertTriptoTripPriceCalculationResponseDTO(seatPrice,
                totalCommissionPerSeat, commissionPercentage);
    ...
    public Response<Void> startTrip(Long tripId) {
        ...
        stateTransitionService.transition(trip, ScopeEnum.TRIP, TripStateEnum.CLOSED.name(),
                TripStateEnum.IN_PROGRESS.name());
    ...
    public Response<Void> cancelTrip(TripCancellRequestDTO tripCancellRequestDTO) {
        ...
        List<Reservation> pendingReservations = reservationRepository.findByTripIdAndStateName(trip.getId(),
        ...
        if (!acceptedReservations.isEmpty() || !pendingReservations.isEmpty()) {
            if (tripCancellRequestDTO.getReason() == null || tripCancellRequestDTO.getReason().isBlank()) {
                throw new ConflictException(
                        "Este viaje cuenta con reservas activas, por lo que tenés que justificar el motivo de su cancelación.");
            }
        ...
        stateTransitionService.transition(trip, ScopeEnum.TRIP,
                List.of(TripStateEnum.CREATED.name(), TripStateEnum.CLOSED.name()), TripStateEnum.CANCELLED.name());
    ...
    public Response<CurrentTripResponseDTO> getCurrentTrip() {
        ...
        Optional<Trip> currentTripOpt = tripRepository.findCurrentTripByDriver(driver.getId());
    ...
    public Response<Void> arriveTripStop(TripArriveRequestDTO tripArriveRequestDTO) {
        ...
        Trip currentTrip = tripRepository.findCurrentTripByDriver(driver.getId())
                .orElseThrow(
                        () -> new EntityNotFoundException("El chofer no tiene un viaje en curso en este momento."));

        TripStop stop = currentTrip.getTripStops().stream()
                .filter(ts -> ts.getId() == tripArriveRequestDTO.getIdTripStop())
                .findFirst()
                .orElseThrow(() -> new EntityNotFoundException("Parada no encontrada en el viaje"));

        validateStopOrderToClose(currentTrip, stop);

        List<Reservation> reservations = reservationRepository
                .findReservationsByTripAndDestinationAndState(currentTrip.getId(), stop.getId(), "IN_PROGRESS");
     ...
     * @throws ConflictException  si el viaje no cumple con las condiciones
     *                            necesarias para ser editado
     */
    private void validateTripEditable(Trip trip) {
        ...
        // Se valida que el horario actual no se encuentre dentro de las 12 horas del
        // inicio del viaje
     ...
     * <p>
     * Este método se utiliza como punto central para recuperar un viaje desde la
     * base
     * de datos, asegurando que exista antes de continuar con cualquier lógica de
     * negocio.
     *
    ...
    /**
     * Metodo encargado de finalizar un viaje, realizando la transicion de estados
     * correspondiente
     * 
    ...
    /**
     * Este metodo permite corroborar si un viaje se encuentra en un determinado
     * estado
     * 
    ...
    /**
     * Metodo que se utiliza para validar el orden de una parda intermedia que se
     * quiere cerrar para un viaje
     * Las paradas se deben cerrar en orden y no se puede cerrar si la anterior no
     * tiene horario de llegada.
     * A su vez no es posible iniciar un vijae con este endpoint, solamente cerrar
     * desde la segunda parada intermedia hasta
     * el destino
     */
    ...
    /**
     * Metodo que permite calcular la nueva fecha que se desea modificar el viaje y
     * corroborar que la misma sea valida, como: que la misma no se encuentre dentro
     * de las 12 horas previas al inicio del viaje, que no se superponga con otro
     * viaje.
     * 
     ...
     */
    private void validateAndApplyNewStartDateTime(LocalDateTime newStartDateTime, Trip trip, Long driverId,
    ...
    private String changeBaggage(String baggageValue) {
        ...
        if (!BaggageEnum.contains(baggageValue)) {
            log.error(
                    "El tipo de equipaje recibo de la request: {} no coincide con los definidos en el enum de equipaje",
                    baggageValue);
